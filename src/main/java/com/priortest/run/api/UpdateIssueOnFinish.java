...
public class UpdateIssueOnFinish {
    ...
    public static void updateIssueIds(File javaFile, String methodName, String[] newIssueIds) {
        ...
        try {
            ...
            log.info("----------------------" +methodName +"------------------" + Arrays.toString(newIssueIds));
                ...
                if (method.getNameAsString().equals(methodName)) {
                            ...
                            normalAnnotation.getPairs().forEach(pair -> {
                                if (pair.getNameAsString().equals("issueId")) {
                                    ...
                                    if (pair.getValue() instanceof ArrayInitializerExpr) {
                                        ...
                                        StringLiteralExpr existingIssueId = (StringLiteralExpr) pair.getValue();
                    ...
                    if (!method.getAnnotationByName("TestCaseApi").isPresent()) {
                        method.addAnnotation(new NormalAnnotationExpr(
                                new Name("TestCaseApi"),  // Corrected: Directly use Name class
